...
@section('content_header')
    ...
    <div class="card">
        <div class="card-body" style="font-family: 'Times New Roman', Times, serif;">
            <strong>Lista de los tipo casos</strong>
            ...
            <table class="table table-striped table-bordered dt-responsive nowrap" style="width:100%" id="tipo_caso">
                <thead class="thead-dark">
                    <tr>
                        ...
                        {{-- @if (auth()->user()->can('tipo_casos.show') || auth()->user()->can('tipo_casos.edit') || auth()->user()->can('tipo_casos.destroy')) --}}
                ...
                <tbody>
                    @foreach ($tipo_casos as $tipo_caso)
                        <tr>
                            ...
                            {{-- <td>{{ ucfirst($tipo_caso->descripcion) }}</td> --}}
                            <td>{{ ucfirst(strip_tags($tipo_caso->descripcion)) }}</td>


                            {{-- <td>{{ ucfirst($tipo_mensaje->descripcion) }}</td> --}}

                            {{-- @if ($tipo_caso->foto == null)

                            <td><a href="#" data-toggle="modal" data-target="#contenidoModal{{ $tipo_caso->id }}">Contenido Específico</a></td>
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <div style="margin-left: 20px;">
                                                    <b>{{ ucfirst($tipo_caso->nombre) }}</b> <br> <br>
                                                    {{ ucfirst(auth()->user()->name) }}
                                                    {{ ucfirst(auth()->user()->apellidopaterno) }}&nbsp;<br> <br>
                                                    {!! $tipo_caso->descripcion !!}
                                                </div>
                            ...
                            @else
                                <td><img class="img-thumbnail" width="70" height="70"
                                        src="{{ Storage::url($tipo_caso->foto) }}" alt="" data-toggle="modal"
                                        data-target="#imagenModal{{ $tipo_caso->id }}">
                                </td>
                                {{-- Modal para mostrar la imagen 
                                <div class="modal fade" id="imagenModal{{ $tipo_caso->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="imagenModal{{ $tipo_caso->id }}Label"
                                    aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="imagenModal{{ $tipo_caso->id }}Label">
                                                </h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                            ...
                                            <div class="modal-body text-center">
                                                <img src="{{ Storage::url($tipo_caso->foto) }}" class="img-fluid"
                                                    alt="">
                            ...
                            @if ($tipo_caso->estado == 1)
                                <td>activo</td>
                            @else
                                <td>inavilitado</td>
                            @endif

                            {{-- <td style="@if ($tipo_caso->estado == 0) background-color: rgba(255, 0, 0, 0.3); @endif">
                                {{ ucfirst($tipo_caso->fecha) }}</td> --}}
                            ...
                            <td>
                                ...
                                <form action="{{ route('tipo_casos.destroy', $tipo_caso) }}" method="POST"
                                    class="form-eliminar">
                                    @csrf
                                    @method('DELETE')
